...
export const Sidebar = ({ onRoomJoined }) => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      addToast(`"${data.room.name}" created! Code: ${data.room.code}`, 'success');
      await joinRoom(data.room);
    } catch (err) { addToast(err.response?.data?.error || 'Failed to create', 'error'); }
  ...
  const handleJoinCode = async (e) => {
    ...
    try {
      ...
      setShowJoin(false);
      setJoinCode('');
    } catch { addToast('Room not found', 'error'); }
  };
  ...
  const joinRoom = async (room) => {
    ...
    try {
      ...
      addToast('Camera/mic access failed. Check permissions.', 'error');
    } finally { setJoiningRoom(null); }
  };

  const handleLogout = () => {
    if (inCall) { getSocket().emit('leave_room'); useCallStore.getState().cleanupCall(); }
    disconnectSocket();
    logout();
    navigate('/login');
  };
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-secondary)', borderRight: '1px solid var(--border)' }}>
      ...
      {/* Quick actions */}
      <div className="px-3 py-2 flex-shrink-0 space-y-0.5" style={{ borderBottom: '1px solid var(--border)' }}>
        <button onClick={() => { setShowCreate(v => !v); setShowJoin(false); }} className="sidebar-item w-full text-left">
          <div className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0" style={{ background: 'var(--accent-dim)' }}>
            <Plus size={12} style={{ color: 'var(--accent)' }} />
          </div>
          <span className="text-sm">Create room</span>
        </button>
        <button onClick={() => { setShowJoin(v => !v); setShowCreate(false); }} className="sidebar-item w-full text-left">
          <div className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 bg-white/6">
      ...
      {/* Create form */}
      {showCreate && (
        <form onSubmit={handleCreate} className="px-3 py-3 space-y-2 animate-slide-up flex-shrink-0"
          ...
          <div className="flex gap-2">
            {['open', 'private'].map(t => (
              <button key={t} type="button"
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-xl text-xs font-medium transition-all ${createForm.type === t ? 'text-amber-400' : 'text-slate-500 hover:text-white'}`}
                style={{
      ...
      {/* Join by code */}
      {showJoin && (
        <form onSubmit={handleJoinCode} className="px-3 py-3 space-y-2 animate-slide-up flex-shrink-0"
          style={{ borderBottom: '1px solid var(--border)' }}>
      ...
      <div className="flex-1 overflow-y-auto py-3 min-h-0">
        <div className="px-4 mb-2 flex items-center justify-between">
          ...
          <button onClick={loadRooms} className="w-6 h-6 rounded-lg flex items-center justify-center transition-all hover:bg-white/6"
            ...
            {rooms.map(room => {
              ...
              return (
                <button key={room.roomId} onClick={() => joinRoom(room)} disabled={isJoining}
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl transition-all duration-150 text-left group ${isActive ? 'text-amber-400' : 'text-slate-400 hover:text-white'}`}
                  style={{
                  ...
                  {isJoining ? <Spinner size="sm" /> : (
                    <ChevronRight size={12} className="opacity-0 group-hover:opacity-50 transition-opacity flex-shrink-0" />
                  )}
                </button>
              );
            })}
      ...
      {/* Status editor — always accessible */}
      <StatusEditor />

      {/* User footer */}
      <div className="px-3 py-2.5 flex items-center gap-3 flex-shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
        <div className="relative flex-shrink-0">
          <Avatar name={user?.displayName || '?'} size="sm" />
          <div className="online-dot" style={{ width: 8, height: 8 }} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-white truncate" style={{ fontFamily: 'Syne, sans-serif' }}>
